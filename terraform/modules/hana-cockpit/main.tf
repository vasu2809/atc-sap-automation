...
resource "google_compute_attached_disk" "data" {
  ...
  instance    = element(split("/", element(tolist(module.sap_hana_cockpit.instances_self_links), 0)), 10)
  device_name = "${element(split("/", element(tolist(module.sap_hana_cockpit.instances_self_links), 0)), 10)}-data"
  project     = var.project_id
  zone        = var.zone
}
...
resource "google_compute_attached_disk" "backup" {
  ...
  instance    = element(split("/", element(tolist(module.sap_hana_cockpit.instances_self_links), 0)), 10)
  device_name = "${element(split("/", element(tolist(module.sap_hana_cockpit.instances_self_links), 0)), 10)}-backup"
  project     = var.project_id
  zone        = var.zone
}
