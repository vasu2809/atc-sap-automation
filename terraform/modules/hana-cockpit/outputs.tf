...
output "instances_self_links" {
  value = module.sap_hana_cockpit.instances_self_links
}
...
output "instance_group_link" {
  value = length(module.sap_hana_cockpit.instances_self_links) != 0 ? element(tolist(module.sap_hana_cockpit.instances_self_links), 0) : ""
}

output "instance_name" {
  value = length(module.sap_hana_cockpit.instances_self_links) != 0 ? element(split("/", element(tolist(module.sap_hana_cockpit.instances_self_links), 0)), 10) : ""
}
